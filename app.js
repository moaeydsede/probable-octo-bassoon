...
function normalizeState(){
  // توافق خلفي للسندات: تحويل السند القديم (حساب واحد) إلى أسطر متعددة
  state.vouchers = (state.vouchers||[]).map(v=>{
    if(Array.isArray(v.lines) && v.lines.length){
      const total = v.lines.reduce((t,l)=>t+Number(l.amount||0),0);
      return { ...v, total: Number(v.total||total)||total };
    }
    const line = { account: v.account||"", party: v.party||"", amount: Number(v.amount||0), note: v.note||"" };
    return { ...v, lines:[line], total: Number(v.amount||0) };
  });
}
...
const NAV = [
  ...
  { id:"accounts", title:"دليل الحسابات", view:"view_accounts", roles:["admin","user"], section:"ملفات" },
  { id:"items", title:"دليل المواد", view:"view_items", roles:["admin","user"], section:"ملفات" },
  ...
  { id:"gl", title:"دفتر الأستاذ", view:"view_gl", roles:["admin","user"], section:"التقارير" },
  { id:"statement", title:"كشف حساب", view:"view_statement", roles:["admin","user"], section:"التقارير" },
  { id:"tb", title:"ميزان المراجعة", view:"view_tb", roles:["admin","user"], section:"التقارير" },
  { id:"balances", title:"أرصدة العملاء/الموردين", view:"view_balances", roles:["admin","user"], section:"التقارير" },
  { id:"cogs", title:"تكلفة البضاعة المباعة", view:"view_cogs", roles:["admin","user"], section:"التقارير" },
...
function voucherCardHTML(v){
  ...
  const pill = `${v.type} • ${money(total)}`;
  const lines = Array.isArray(v.lines) ? v.lines : [];
  const partiesAll = [...new Set(lines.map(l=>(l.party||"").trim()).filter(Boolean))];
  const parties = partiesAll.slice(0,2);
  const partyTxt = parties.length ? `— ${parties.join("، ")}${(partiesAll.length>2)?"…":""}` : (v.party?`— ${v.party}`:"");

  const firstAcc = lines[0]?.account || v.account || "";
  const accLabel = firstAcc ? `${firstAcc} — ${accountName(firstAcc)}` : "";
    ...
    <div class="doc-card">
      <div class="doc-head">
        <div>
          ...
          <div class="doc-party">${accLabel} ${partyTxt}</div>
        </div>
      ...
      <div class="doc-meta">
        ...
        <span class="pill">بنود: ${lines.length || 1}</span>
      </div>
      <div class="doc-actions">
...
function nextVoucherNo(){
  const max = (state.vouchers||[]).reduce((m,v)=>Math.max(m, Number(v.no||0)), 0);
...
function applyVoucherEffects(v){
  ...
  const cc = v.cc || "";
  const lines = (v.lines||[]).filter(l=>l && l.account && Number(l.amount||0)>0);
  ...
  if(v.type==="قبض"){
    postJournal({
      date,
      note:`سند قبض (${lines.length} بند)`,
      system:true,
      docRef: ref,
      lines: [
        { account: cashAcc, debit: total, credit:0, cc },
        ...lines.map(l=>({ account: l.account, debit:0, credit:Number(l.amount||0), cc }))
      ]
    });
  }else{
    postJournal({
      date,
      note:`سند صرف (${lines.length} بند)`,
      system:true,
      docRef: ref,
      lines: [
        ...lines.map(l=>({ account: l.account, debit:Number(l.amount||0), credit:0, cc })),
        { account: cashAcc, debit:0, credit: total, cc }
      ]
    });
  }
}
...
function renderVouchers(){
  ...
  const rows=[...state.vouchers].filter(v=>{
    const lines = Array.isArray(v.lines)?v.lines:[];
    const accStr = (lines.map(l=>l.account).join(" ") || v.account || "");
    const partyStr = (lines.map(l=>l.party).join(" ") || v.party || "");
    const noteStr = (v.note||"") + " " + (lines.map(l=>l.note||"").join(" "));
    return !q || (String(v.no)+v.type+accStr+partyStr+noteStr+v.date).includes(q);
  }).slice().reverse();
  ...
  for(const v of rows){
    ...
    tr.innerHTML=`
      ...
      <td>${v.type}</td>
      <td class="mono">${accLabel}${lines.length>1?` <span class="pill">+${lines.length-1}</span>`:""}</td>
      <td class="mono"><b>${money(total)}</b></td>
      <td>${v.note||""}</td>
      <td class="ta-left"><div class="chips">
...
function openVoucherModal(no){
  const isEdit=!!no;
  const v=isEdit ? state.vouchers.find(x=>x.no===no) : { no:nextVoucherNo(), date:todayISO(), type:"قبض", cashAccount:"1100", total:0, note:"", cc:"", lines:[{account:"2200", party:"", amount:0, note:""}] };

  // تأكد من وجود lines
  const lines = (Array.isArray(v.lines) && v.lines.length)
    ? v.lines.map(l=>({ account:l.account||"", party:l.party||"", amount:Number(l.amount||0), note:l.note||"" }))
    : [{ account: v.account||"2200", party: v.party||"", amount: Number(v.amount||0), note: v.note||"" }];

  const computeTotal = ()=> lines.reduce((t,l)=>t+Number(l.amount||0),0);
  const collectVoucherLines = ()=> lines
      .map(l=>({ account:(l.account||"").trim(), party:(l.party||"").trim(), amount:Number(l.amount||0), note:(l.note||"").trim() }))
  ...
  const body=`
    <div class="form-grid">
      ...
      <div class="field"><label>الإجمالي</label><input id="m_v_total" type="text" value="${money(computeTotal())}" disabled /></div>

      <div class="field" style="grid-column:1/-1">
        <label>بنود السند (يمكن أكثر من حساب)</label>
        <div id="m_v_lines" class="v-lines"></div>
        <div style="display:flex; gap:8px; margin-top:10px; flex-wrap:wrap">
          <button class="btn" id="m_v_addLine" type="button">+ إضافة بند</button>
          <div class="muted small">ضع اسم الطرف (عميل/مورد) ليظهر في تقرير الأرصدة.</div>
        </div>
      </div>

      <div class="field" style="grid-column:1/-1"><label>ملاحظة عامة</label><input id="m_v_note" type="text" value="${v.note||""}" placeholder="مثال: سند قبض مجمع..." /></div>
    </div>`;

  function updateTotal(){
    $("#m_v_total").value = money(computeTotal());
  }
  ...
  function lineRowHTML(i, l){
    ...
    return `
      <div class="v-line" data-i="${i}">
        <div class="field">
          <label>الحساب</label>
          <select class="m_v_acc"></select>
        </div>
        <div class="field">
          <label>الطرف (اختياري)</label>
          <input class="m_v_party" type="text" value="${(l.party||"")}" placeholder="عميل/مورد"/>
        </div>
        <div class="field">
          <label>المبلغ</label>
          <input class="m_v_amount" type="number" step="0.01" value="${amt}" />
        </div>
        <div class="field">
          <label>بيان (اختياري)</label>
          <input class="m_v_lnote" type="text" value="${(l.note||"")}" placeholder="مثال: سداد..." />
        </div>
        <div class="v-line-actions">
          <button class="btn danger m_v_remove" type="button">حذف</button>
        </div>
      </div>
    `;
  }
  ...
  function renderLines(){
    const wrap = $("#m_v_lines");
    wrap.innerHTML = lines.map((l,i)=>lineRowHTML(i,l)).join("");
    wrap.querySelectorAll(".v-line").forEach(row=>{
      const i = Number(row.dataset.i);

      const sel = row.querySelector(".m_v_acc");
      pickAccountOptions(sel, false);
      if(!lines[i].account){
        lines[i].account = ($("#m_v_type").value==="قبض" ? "2200" : "2100");
      }
      sel.value = lines[i].account;
      sel.addEventListener("change", ()=>{ lines[i].account = sel.value; });

      row.querySelector(".m_v_party").addEventListener("input", (e)=>{ lines[i].party = e.target.value; });
      row.querySelector(".m_v_amount").addEventListener("input", (e)=>{ lines[i].amount = parseMoney(e.target.value); updateTotal(); });
      row.querySelector(".m_v_lnote").addEventListener("input", (e)=>{ lines[i].note = e.target.value; });

      row.querySelector(".m_v_remove").addEventListener("click", ()=>{
        lines.splice(i,1);
        if(!lines.length) lines.push({account: $("#m_v_type").value==="قبض"?"2200":"2100", party:"", amount:0, note:""});
  ...
  openModal(isEdit?`تعديل سند #${v.no}`:"سند جديد", body, [
    { text:"حفظ", kind:"primary", onClick: ()=>{
      ...
      const rows = collectVoucherLines();
      if(!rows.length){ toast("أضف بند واحد على الأقل"); return; }
      const total = rows.reduce((t,l)=>t+Number(l.amount||0),0);
      if(total<=0){ toast("الإجمالي غير صحيح"); return; }

      const v2={...v, date, type, cashAccount, cc, note, lines: rows, total};
      ...
      saveState(state); closeModal(); toast("تم حفظ السند"); renderVouchers(); renderJournal(); renderDashboard();
    }},
    { text:"طباعة", onClick: ()=>{
      const date=$("#m_v_date").value || todayISO();
      const type=$("#m_v_type").value;
      ...
      const rows = collectVoucherLines();
      const total = rows.reduce((t,l)=>t+Number(l.amount||0),0);
      printVoucherObject({...v, date, type, cashAccount, cc, note, lines: rows, total});
    }},
    { text:"إغلاق", onClick: closeModal }
  ]);
  ...
  $("#m_v_addLine").addEventListener("click", ()=>{
    lines.push({ account: $("#m_v_type").value==="قبض"?"2200":"2100", party:"", amount:0, note:"" });
    renderLines(); updateTotal();
  });
...
function runBalances(){
  ...
  const voucherLines = (type, accountId, partyName)=> {
    return vouchers
      .filter(v=>v.type===type)
      .flatMap(v=>{
        const lines = Array.isArray(v.lines)?v.lines:[{account:v.account, party:v.party, amount:v.amount, note:v.note}];
        return lines.map(l=>({ date:v.date, cashAccount:v.cashAccount, cc:v.cc, ...l, vno:v.no, vnote:v.note, vtype:v.type }));
      })
      .filter(l=> (l.account===accountId) && ((l.party||"")===partyName));
  };
  ...
  for(const name of custNames){
    ...
    const r = voucherLines("قبض","2200",name).reduce((t,l)=>t+Number(l.amount||0),0);
    if(s===0 && r===0) continue;
  ...
  for(const name of supNames){
    ...
    const pay = voucherLines("صرف","2100",name).reduce((t,l)=>t+Number(l.amount||0),0);
